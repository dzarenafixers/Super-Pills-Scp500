...
namespace SCP500Expanded.Items
{
    ...
    public class TeleportationPill : CustomItem
    {
        public override uint Id { get; set; } = 5011;
        public override string Name { get; set; } = "Teleportation Pill";
        public override string Description { get; set; } = "Teleports you to a random safe location.";
        ...
        private void OnUse(UsedItemEventArgs ev)
        {
            ...
            var randomRoom = Room.List.Where(room => room.Zone != ZoneType.Surface).ToList().GetRandomValue();
            ev.Player.Teleport(randomRoom.Position + UnityEngine.Vector3.up);
            ev.Player.Broadcast(5, "You have been teleported to a random location!");
        }
    }
}